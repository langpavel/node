...
and can be `'binary'`, `'base64'` or `'hex'`. Defaults to `'buffer'`.
...
`'binary'`, `'base64'` or `'hex'`. Defaults to `'buffer'`.
...
or `'hex'`. Defaults to `'buffer'`.

The `output_decoding` specifies in what format to return the deciphered
plaintext: `'binary'`, `'ascii'` or `'utf8'`. Defaults to `'buffer'`.
...
`'base64'`. The input encoding defaults to `'buffer'`.
